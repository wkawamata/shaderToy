...
float planeIntersect(vec3 ro, vec3 rd, Plane plane) {
    ...
    float t = dot(plane.position - ro, plane.normal) / denom;
    return (t > 0.001) ? t : -1.0;
}

// -------------------- Sky / Environment --------------------
vec3 skyColor(vec3 rd) {
    // rd.y を 0.0～1.0 に正規化してグラデーション
    float t = clamp(rd.y * 0.5 + 0.5, 0.0, 1.0);
    vec3 horizon = vec3(0.8, 0.8, 0.9); // 地平線の淡い色
    vec3 zenith  = vec3(0.2, 0.5, 0.9); // 天頂の青
...
vec2 dirToUV(vec3 dir) {
    ...
    // 0～1に正規化
    return vec2(0.5 - phi / (2.0 * PI), 0.5 + theta / PI); //openGL UVなのでVは上下反転
}

vec3 skyColorEnvmap(vec3 rd) {
    vec2 uv = dirToUV(normalize(rd));
...
bool sceneIntersect(vec3 ro, vec3 rd, out vec3 hitPos, out vec3 normal, out Material mat) {
    ...
    // ---- Scene objects ----
    Sphere spheres[3];
    spheres[0] = Sphere(vec3(2,0,0), 1.0, Material(vec3(0.8,0.3,0.3), 0.0, 0.0)); // diffuse red
    spheres[1] = Sphere(vec3(0,0,0), 1.0, Material(vec3(0.3,0.3,0.9), 1.0, 0.1)); // glossy blue metal
    spheres[2] = Sphere(vec3(-2,0,0), 1.0, Material(vec3(0.3,0.8,0.3), 0.0, 0.0)); // diffuse green

    // --- Floor plane (y = -1) ---
    Plane floorPlane = Plane(vec3(0.0, -1.0, 0.0), vec3(0.0, 1.0, 0.0), Material(vec3(0.9), 0.0, 0.0));
    ...
    {
        // 床との交差判定
        float t = planeIntersect(ro, rd, floorPlane);
        if(t > 0.001 && t < tMin) {
            tMin = t;
            hit = true;
            hitPos = ro + rd * t;
            ...
            vec3 col = mix(vec3(0.9), vec3(0.1), checker);
            mat = Material(col, 0.0, 0.0);
...
vec3 radiance(vec3 ro, vec3 rd, vec2 fragCoord) {
    ...
    for(int bounce = 0; bounce < 8; bounce++) {
        ...
        if(!sceneIntersect(ro, rd, hitPos, normal, mat)) {
            ...
            // sky color
            col += throughput * skyColorEnvmap(rd) * 0.5; // ←変更点
            break;
        }
